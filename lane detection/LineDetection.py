# -*- coding: utf-8 -*-
"""
Created on Tue Mar  3 20:42:53 2020

@author: Ar
"""

import LaneProcessing as lp
import numpy as np
from Line import Line
from LineSearch import LineSpace
import cv2
from utils import funcSpace, putText, curvature, promptForInt, promptForFloat
...
class LineDetector:
    def __init__(self, imgMarginWidth=320, historyDepth=5, margin=100, windowSplit=2, winCount=9, searchPortion=1.):
        self.imgProcessor = lp.Processing()
        self.warper = lp.Warping()
    ...
    def preProcess(self, src):
        ...
        binary = lp.Thresholding.combiThreshold(src=src)
    ...
    def addLanePoly(self, srcShape, dst, fitLeft, fitRight):
        ...
        """
        lanePolyImg, leftOutstand, _ = lp.Drawing.addPolygon(srcShape=srcShape, lFit=fitLeft,
    ...
    def addOffsetStamp(self, leftFit, rightFit, image, origin, color=(255, 255, 255), fontScale=1.0):
        ...
        yBottom = imgH - 1
        cameraCenter = imgW / 2
        lBottomX = funcSpace(argSpace=yBottom, fitParams=leftFit) - self.imgMarginWidth
        rBottomX = funcSpace(argSpace=yBottom, fitParams=rightFit) - self.imgMarginWidth
        laneWidth = rBottomX - lBottomX
        scaleX = 3.7 / laneWidth
        laneCenter = (lBottomX + rBottomX) / 2
        offSet = (cameraCenter - laneCenter) * scaleX
        putText(img=image, text='Estimated Vehicle Offset: {:.2f} m'.format(offSet),
    ...
    def addCurvatureStamp(leftFit, rightFit, image, origin, color=(255, 255, 255), fontScale=1.0):
        ...
        leftCurvature = curvature(fitParams=leftFit, variable=yBottom, scale=scaleY)
        rightCurvature = curvature(fitParams=rightFit, variable=yBottom, scale=scaleY)
        curvature_ = (leftCurvature + rightCurvature) / 2
        putText(img=image, text='Estimated Lane Curvature: {:.1f} m'.format(round(curvature_ / 100, 1) * 100),
                    origin=origin, color=color, scale=fontScale)
...
def main():
    ...
    depth = promptForInt(message='Enter history depth in frames: ') #number of line to keep in fits list
    margin = promptForInt(message='Enter Search Margin: ') #the margin in pixels denoting the area od searchto the left and right from the center line
    fillerWidth = promptForInt(message='Enter filler width: ') 
    windowSplit = promptForInt(message='Enter Window Split: ') # denote which part of the image to use histogram evaluation
    winCount = promptForInt(message='Enter Window Count for Box Search: ') #number of window to split the image height into for initial box search
    searchPortion = promptForFloat(message='Enter the Search portion (0.0 - 1.0): ') #the vertical share of the image usd for searching

    pipAlpha = promptForFloat(message='Enter Picture-in-picture alpha: (0.0 - 1.0): ')
    pipScaleRatio = promptForFloat(message='Enter Picture-in-picture scale (0.0 - 1.0): ')
