# -*- coding: utf-8 -*-
"""
Created on Tue Mar  3 20:40:40 2020
...
from utils import drawRect, funcSpace
...
class LaneFinding:
    ...
    def primarySearchPolyMargin(self, src, lineSpace, winCount, margin, minpix,
        ...
        srcRgb = lp.Drawing.addLine(src=srcRgb, fit=fit, color=[255, 0, 255], thickness=10)
    ...
    def secondarySearch(self, imgH, previousFit, nzX, nzY, margin, src, ratio=1., lineData=None):
        ...
        leftX = funcSpace(argSpace=nzY, fitParams=leftFit)
        rightX = funcSpace(argSpace=nzY, fitParams=rightFit)
        ...
        if lineData is not None:
            ...
            for lineFit in lineData['fits']:
                srcRgb = lp.Drawing.addLine(src=srcRgb, fit=lineFit, color=color, thickness=3, stepCount=20)

        srcRgb = lp.Drawing.addLine(src=srcRgb, fit=fit, color=[255, 0, 255], thickness=10, stepCount=20)

        winImg = np.zeros_like(srcRgb)

        y = np.linspace(start=0, stop=imgH - 1, num=imgH)

        xl = funcSpace(argSpace=y, fitParams=leftFit)
        xr = funcSpace(argSpace=y, fitParams=rightFit)
    ...
    def borderFit(imgH, centralFit, margin):
        ...
        y = np.linspace(start=0, stop=imgH - 1, num=imgH)
        x = funcSpace(argSpace=y, fitParams=centralFit)
